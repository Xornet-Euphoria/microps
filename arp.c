...
arp_dump(const uint8_t *data, size_t len)
{
    ...
    fprintf(stderr, "hln: %u", message->hdr.hln);
    fprintf(stderr, "pln: %u", message->hdr.pln);
...
arp_resolve(struct net_iface *iface, ip_addr_t pa, uint8_t *ha)
{
    ...
    if (!cache) {
        ...
        pthread_mutex_unlock(&mutex);
        debugf("cache not found, pa=%s, ha=%s", ip_addr_ntop(pa, addr1, sizeof(addr1)), ether_addr_ntop(ha, addr2, sizeof(addr2)));
