...
udp_pcb_queue_pop(struct udp_pcb *pcb)
{
    ...
    while (!net_interrupt_occurred(ctx)) {
        ...
        timespec_add_nsec(&timeout, 100000000);
        pcb->wait++;
...
udp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    debugf("%s:%d => %s:%d, len=%zu (payload=%zu)", ip_addr_ntop(src, addr1, sizeof(addr1)), ntoh16(hdr->src), ip_addr_ntop(dst, addr2, sizeof(addr2)), ntoh16(hdr->dst), len, len - sizeof(*hdr));
    udp_dump(data, len);
    ...
    memcpy(entry + 1, hdr + 1, entry->len); // data follows after entry
...
udp_output(struct udp_endpoint *src, struct udp_endpoint *dst, const  uint8_t *data, size_t len)
{
    ...
    if (ip_output(IP_PROTOCOL_UDP, (u_int8_t *)hdr, total, src->addr, dst->addr) == -1) {
...
udp_bind(int id, struct udp_endpoint *local)
{
    ...
    if (exist) {
        errorf("already exists: local=%s", udp_endpoint_ntop(local, ep1, sizeof(ep1)));
        pthread_mutex_unlock(&mutex);
        return -1;
    }
