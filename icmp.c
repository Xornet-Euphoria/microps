...
{
    struct icmp_hdr *hdr;
    struct icmp_echo *echo_hdr;

    flockfile(stderr);
    hdr = (struct icmp_hdr *)data;
    fprintf(stderr, "type: %u (%s)\n", hdr->type, icmp_type_ntoa(hdr->type));
    fprintf(stderr, "code: %u\n", hdr->code);
    fprintf(stderr, "sum: 0x%4x\n", ntoh16(hdr->sum));
    switch (hdr->type) {
    case ICMP_TYPE_ECHOREPLY:
    case ICMP_TYPE_ECHO:
        echo_hdr = (struct icmp_echo *)hdr;
        fprintf(stderr, "id: %u\n", ntoh16(echo_hdr->id));
...
icmp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    debugf("%s => %s, len=%zu", ip_addr_ntop(dst, addr1, sizeof(addr1)), ip_addr_ntop(src, addr2, sizeof(addr2)), len);
    icmp_dump(data, len);
